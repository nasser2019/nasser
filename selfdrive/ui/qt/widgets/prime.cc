...
PrimeUserWidget::PrimeUserWidget(QWidget* parent) : QWidget(parent) {
  ...
  QLabel* subscribed = new QLabel("✓ مشترك");
  ...
  QLabel* commaPrime = new QLabel("كوما الممتازة");
  ...
  QLabel* connectUrl = new QLabel("تواصل مع كوما.للذكاء الاصطناعي");
...
PrimeAdWidget::PrimeAdWidget(QWidget* parent) : QFrame(parent) {
  ...
  QLabel *upgrade = new QLabel("قم بالترقية الآن");
  upgrade->setStyleSheet("font-size: 75px; font-weight: bold;");
  main_layout->addWidget(upgrade, 0, Qt::AlignTop);
  main_layout->addSpacing(50);

  QLabel *description = new QLabel("كن عضو مميز في كوما تواصل مع كوما.للذكاء الاصطناهي");
  ...
  QLabel *features = new QLabel("مزايا العضوية:");
  features->setStyleSheet("font-size: 41px; font-weight: bold; color: #E5E5E5;");
  main_layout->addWidget(features, 0, Qt::AlignBottom);
  main_layout->addSpacing(30);

  QVector<QString> bullets = {"الوصول عن بعد", "14 يوما من التخزين", "امتيازات المطور"};
  for (auto &b: bullets) {
...
SetupWidget::SetupWidget(QWidget* parent) : QFrame(parent) {
  ...
  QLabel* registrationDescription = new QLabel("قم بإقران جهازك بكوما للاتصال (connect.comma.ai) واطلب عرض كوما الأساسي الخاص بك.");
  ...
  QPushButton* pair = new QPushButton("إقران الجهاز");
...
void SetupWidget::replyFinished(const QString &response, bool success) {
  ...
  QJsonDocument doc = QJsonDocument::fromJson(response.toUtf8());
  if (doc.isNull()) {
    qDebug() << "فشل تحليل الاشتراك في الحصول على الاقتران والحالة الأولية";
    return;
  }
  ...
  if (uiState()->prime_type != prime_type) {
    ...
    Params().put("نوع العضوية", std::to_string(prime_type));
  }

  if (!json["تم_الارتباط"].toBool()) {
